...
function Wrap() {
  ...
  const onChangeWrap = async action => {
    ...
    if (
      ...
      return dispatch(error("You cannot wrap more than your sMDAO balance."));
    }
    ...
    if (
      ...
      return dispatch(error("You cannot unwrap more than your wsMDAO balance."));
    }
  ...
  const assetName = asset === 0 ? "wsMDAO" : "gMDAO";
    ...
    if (view === 0 && asset === 0)
      return (
        <div className="no-input-visible">
          Wrapping to <b>wsMDAO</b> is disabled at this time due to the upcoming{" "}
          <a className="v2-migration-link" href="https://olympusdao.medium.com/introducing-olympus-v2-c4ade14e9fe">
            V2 migration
          </a>
          .
          <br />
          If you'd like to wrap your <b>sMDAO</b>, please try wrapping to <b>gMDAO</b> instead.
        </div>
      );
    if (!hasAllowance("sohm") && view === 0 && asset === 1)
      return (
        <div className="no-input-visible">
          First time wrapping to <b>gMDAO</b>?
          <br />
          Please approve Olympus Dao to use your <b>sMDAO</b> for this transaction.
        </div>
      );
    if (!hasAllowance("gohm") && view === 1 && asset === 1)
      return (
        <div className="no-input-visible">
          First time unwrapping <b>gMDAO</b>?
          <br />
          Please approve Olympus Dao to use your <b>gMDAO</b> for unwrapping.
        </div>
      );
    if (!hasAllowance("wsohm") && view === 1 && asset === 0)
      return (
        <div className="no-input-visible">
          First time unwrapping <b>wsMDAO</b>?
          <br />
          Please approve Olympus Dao to use your <b>wsMDAO</b> for unwrapping.
        </div>
      );
    ...
    if (view === 0) {
      ...
      if (hasAllowance("sohm") && asset === 1)
        return (
          ...
          >
            {txnButtonText(pendingTransactions, "wrapping", "Wrap to gMDAO")}
          </Button>
        );
    }
    // unwrap view
    if (view === 1) {
      ...
      if (hasAllowance(assetName.toLowerCase()))
        return (
          ...
          >
            {asset === 0
              ? txnButtonText(pendingTransactions, "unwrapping", "Unwrap wsMDAO")
              : txnButtonText(pendingTransactions, "unwrapping", "Unwrap gMDAO")}
          </Button>
        );
    }
  };
                ...
                <Grid container spacing={2} alignItems="flex-end">
                  ...
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                    <div className="wrap-index">
                      ...
                      <Typography variant="h4">
                        {currentIndex ? <>{trim(currentIndex, 1)} MDAO</> : <Skeleton width="150px" />}
                      </Typography>
                    </div>
                  </Grid>
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                    <div className="wrap-wsOHM">
                      <Typography variant="h5" color="textSecondary">
                        {`${assetName} Price`}
                        <InfoTooltip
                          message={`${assetName} = sMDAO * index\n\nThe price of ${assetName} is equal to the price of MDAO multiplied by the current index`}
                        />
                      </Typography>
                      <Typography variant="h4">
            ...
            <div className="staking-area">
              {!address ? (
                <div className="stake-wallet-notification">
                  ...
                  <Typography variant="h6">Connect your wallet to wrap sMDAO</Typography>
                </div>
              ) : (
                <>
                    ...
                    <Box>
                      <FormControl style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
                        ...
                        <Select id="asset-select" value={asset} label="Asset" onChange={changeAsset} disableUnderline>
                          <MenuItem value={0}>wsMDAO</MenuItem>
                          <MenuItem value={1}>gMDAO</MenuItem>
                        </Select>
                      </FormControl>
                    </Box>
                    ...
                    {quantity && (
                      <Box padding={1}>
                        <Typography variant="body2" className={classes.textHighlight}>
                          {isUnwrap
                            ? `Unwrapping ${quantity} ${asset === 0 ? "wsMDAO" : "gMDAO"} will result in ${trim(
                                convertedQuantity,
                                4,
                              )} sMDAO`
                            : `Wrapping ${quantity} sMDAO will result in ${trim(convertedQuantity, 4)} gMDAO`}
                        </Typography>
                      </Box>
                    )}
                  </Box>
                  ...
                  <div className={`stake-user-data`}>
                    <div className="data-row">
                      ...
                      <Typography variant="body1">
                        {isAppLoading ? <Skeleton width="80px" /> : <>{trim(sohmBalance, 4)} sMDAO</>}
                      </Typography>
                    </div>
                    <div className="data-row">
                      ...
                      <Typography variant="body1">
                        {isAppLoading ? (
                          ...
                          <>{asset === 0 ? trim(wsohmBalance, 4) + " wsMDAO" : trim(gohmBalance, 4) + " gMDAO"}</>
                        )}
                      </Typography>
                    </div>
                  </div>
